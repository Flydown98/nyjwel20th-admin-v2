...
function isCurrentHallLayout(){return /^(HALL400-N-|ASYM402-|ASYM312-)/.test(str(state?.meta?.seatLayout));}
...
function defaultState() {
  return {
    meta:{app:'nyjwel20th-admin-v2',version:'0.9.37',createdAt:nowIso(),updatedAt:nowIso(),importedAt:null,importSource:null},
    settings:{
    ...
    raffles:[], rouletteHistory:[], rouletteProducts:[], smsQueue:[], logs:[], seatLayoutMemories:[]
  };
}
function normalizeState(s) {
  ...
  return {
    ...d,...(s||{}),
    meta:{...d.meta,...(s?.meta||{}),version:'0.9.37'},
    ...
    seatLayoutMemories:Array.isArray(s?.seatLayoutMemories)?s.seatLayoutMemories:[]
  };
}
function loadState(){
  try{
...
function seedDemoState(){
  ...
  state.settings.autoSeatAssignOnCheckin=false;
...
function seatByCode(code){
  ...
  const existing=(state.seats||[]).find(s=>str(s.code).toUpperCase()===key);
  if(existing)return existing;
  // 최종 400석에 존재하는 좌석이면 아직 state.seats에 저장되지 않았어도 관리자가 즉시 지정 가능.
  return buildHall400Seats().find(s=>str(s.code).toUpperCase()===key)||buildEvent400Seats().find(s=>str(s.code).toUpperCase()===key)||null;
}
...
function buildEvent400Seats(){
  ...
  for(const row of 'MNOPQRST'.split('')){
    for(let n=1;n<=26;n++){
      const priorityEdge=(n<=2||n>=25);
      out.push({code:`${row}B-${String(n).padStart(2,'0')}`,label:`${row}${n}`,row,side:'B',number:n,displayNumber:n,
        section:'rear',block:'REAR-208',zone:priorityEdge?'휠체어우선':'일반석',enabled:true,autoAssignable:true,
        wheelchairAssignable:priorityEdge,wheelchairOnly:priorityEdge,sortOrder:sort++});
    }
  }
  return out;
...
function buildHall400Seats(){
  const out=[];let sort=1;
  // v0.9.32 실제 현장: 앞 A~F는 기존 16석 유지.
  for(const row of 'ABCDEF'.split('')){
    for(let n=1;n<=16;n++){
      const side=n<=8?'L':'R';
      out.push({code:`${row}${n}`,label:`${row}${n}`,row,side,number:n,displayNumber:n,
        section:'front',block:`FRONT-${side}`,zone:['A','B','C'].includes(row)?'우선석':'내빈석',enabled:true,
        autoAssignable:false,wheelchairAssignable:false,wheelchairOnly:false,sortOrder:sort++});
    }
  }
  // 뒤쪽은 런웨이 기준 왼쪽 18석(1~18) + 오른쪽 8석(19~26).
  // 왼쪽은 G~N 8줄, 오른쪽은 G~O 9줄이다.
  for(const row of 'GHIJKLMNO'.split('')){
    for(let n=1;n<=26;n++){
      const left=n<=18, leftExists=row!=='O';
      const enabled=left ? leftExists : true;
      out.push({code:`${row}${n}`,label:`${row}${n}`,row,side:left?'L':'R',number:n,displayNumber:n,
        section:'rearAsym',block:left?'LEFT-18':'RIGHT-8',zone:enabled?'일반석':'막힘',enabled,
        autoAssignable:enabled,wheelchairAssignable:false,wheelchairOnly:false,blocked:!enabled,sortOrder:sort++});
    }
  }
  return out;
}

function remapToAsym312Seat(code){
  const raw=str(code).toUpperCase();if(!raw)return '';
  let m=raw.match(/^([A-F])(\d{1,2})$/);
  if(m){const n=num(m[2]);if(n>=1&&n<=16)return `${m[1]}${n}`;}
  m=raw.match(/^([A-F])([LR])-(\d{1,2})$/);
  if(m){const n=num(m[3]);if(n>=1&&n<=8)return `${m[1]}${m[2]==='L'?n:n+8}`;}
  // v0.9.31: G~N = 왼쪽 18석
  m=raw.match(/^([G-N])(\d{1,2})$/);
  if(m){const n=num(m[2]);if(n>=1&&n<=18)return `${m[1]}${n}`;if(n>=19&&n<=26)return `${m[1]}${n}`;}
  // v0.9.31 오른쪽 별도 줄 O~V 1~8 -> 같은 물리 줄의 19~26
  m=raw.match(/^([O-V])(\d{1,2})$/);
  if(m){const n=num(m[2]);const inv={O:'G',P:'H',Q:'I',R:'J',S:'K',T:'L',U:'M',V:'N'};if(n>=1&&n<=8)return `${inv[m[1]]}${n+18}`;}
  // v0.9.31 오른쪽 9번째 줄 W1~8 -> O19~26
  m=raw.match(/^W(\d{1,2})$/);if(m){const n=num(m[1]);if(n>=1&&n<=8)return `O${n+18}`;}
  // v0.9.30 숫자형 G~N 1~30: 1~18 왼쪽, 19~26 오른쪽. 27~30은 후속 빈자리로 이동.
  m=raw.match(/^([G-N])(\d{1,2})$/);
  if(m){const n=num(m[2]);if(n>=1&&n<=26)return `${m[1]}${n}`;}
  // 구 좌우형
  m=raw.match(/^([G-N])([LR])-(\d{1,2})$/);
  if(m){const n=num(m[3]);if(n<1||n>18)return '';return `${m[1]}${m[2]==='L'?n:n+18}`;}
  m=raw.match(/^([MN])B-(\d{1,2})$/);
  if(m){const n=num(m[2]);if(n>=1&&n<=26)return `${m[1]}${n}`;}
  return '';
}

function applyCurrentHallLayout(){
  const canonical=buildHall400Seats();
  const valid=new Set(canonical.filter(s=>s.enabled!==false).map(s=>str(s.code).toUpperCase()));
  const occupied=new Set();
  let preserved=0,moved=0,cleared=0,fallbackMoved=0;
  const unresolved=[];
  state.participants.filter(participantActive).sort((a,b)=>num(a.receptionNo)-num(b.receptionNo)).forEach(p=>{
    const oldSeat=str(p.seat).toUpperCase();if(!oldSeat)return;
    const target=remapToAsym312Seat(oldSeat);
    if(target&&valid.has(target)&&!occupied.has(target)){
      if(target===oldSeat)preserved++;else moved++;
      p.seat=target;p.seatLocked=true;p.modifiedAt=nowIso();occupied.add(target);return;
    }
    unresolved.push(p);
  });
  const pool=canonical.filter(s=>s.enabled!==false&&s.autoAssignable!==false&&!occupied.has(str(s.code).toUpperCase()))
    .sort((a,b)=>num(a.sortOrder)-num(b.sortOrder));
  unresolved.forEach(p=>{
    const seat=pool.shift();
    if(seat){p.seat=seat.code;p.seatLocked=true;p.modifiedAt=nowIso();occupied.add(seat.code);moved++;fallbackMoved++;}
    else{p.seat='';p.seatLocked=false;p.seatCategory='standing';p.standing=true;p.modifiedAt=nowIso();cleared++;}
  });
  state.seats=canonical;state.settings.autoSeatAssignOnCheckin=true;
  state.meta.seatLayout='ASYM312-V2';state.meta.seatLayoutAppliedAt=nowIso();state.meta.seatPlanVersion='v0.9.33-left18-right8-26';
  return {preserved,moved,cleared,fallbackMoved,seats:canonical.filter(s=>s.enabled!==false).length,totalCells:canonical.length,participantSeats:312,staffSeats:0};
}
...
function participantSeatLabel(p){
  if(p?.seat)return displaySeatCode(p.seat);
  return p?.seatCategory==='standing'||p?.standing?'스탠딩석':'자유석';
}
function applyStandingPreference(p){
  if(!p)return;
  if(p.seatCategory==='standing'){p.seat='';p.seatLocked=false;p.standing=true;}
  else if(p.standing===true){p.standing=false;}
}
...
function displaySeatCode(code){
  ...
  const n=raw.match(/^([A-W])(\d{1,2})$/);return n?`${n[1]}${num(n[2])}`:raw;
}
...
function assignOne(p, excludeIds=[]){
  if(p.seatCategory==='standing'){applyStandingPreference(p);return '';}
...
function wheelchairPairCandidates(excludeIds=[]){
  const occ=occupiedSeatSet(excludeIds);
  const hall=isCurrentHallLayout();
  const rows=(hall?'MN':'MNOPQRST').split(''),out=[];
  rows.forEach(row=>{
    const code=n=>`${row}B-${String(n).padStart(2,'0')}`;
    if(hall){
      out.push({row,side:'left',wheel:code(1),guardian:code(2),general:Array.from({length:26},(_,i)=>code(i+3))});
      out.push({row,side:'right',wheel:code(30),guardian:code(29),general:Array.from({length:26},(_,i)=>code(28-i))});
    }else{
      out.push({row,side:'left',wheel:code(1),guardian:code(2),general:Array.from({length:22},(_,i)=>code(i+3))});
      out.push({row,side:'right',wheel:code(26),guardian:code(25),general:Array.from({length:22},(_,i)=>code(24-i))});
    }
  });
...
function assignGroupSmart(people){
  const active=people.filter(Boolean);
  active.filter(p=>p.seatCategory==='standing').forEach(applyStandingPreference);
  const need=active.filter(p=>!p.seat&&p.seatCategory!=='standing');
  if(need.some(p=>p.wheelchairUser||p.seatCategory==='wheelchair')){
    const r=assignWheelchairParty(need,{lock:false});
    return {...r,seats:active.map(p=>p.seat).filter(Boolean),standing:active.filter(p=>p.seatCategory==='standing').length};
  }
  assignContiguousCategory(need,'general');
  return {seats:active.map(p=>p.seat).filter(Boolean),mixed:false,wheelchair:0,companions:need.filter(p=>p.seat).length,standing:active.filter(p=>p.seatCategory==='standing').length};
}
...
function checkinMessage(p,extra=''){
  const hasSeat=Boolean(str(p?.seat));
  const seat=hasSeat?displaySeatCode(p.seat):'자유석';
  const guideLine=hasSeat?`
좌석배치도: ${seatGuideUrl(p)}`:'';
  return `[남양주시장애인복지관]
...
function getGroupExclusionKeywords(){
  // v0.9.28: 행사 운영 기준으로 고정. 과거 JSON에 저장된 넓은 제외어(남양주시/낮활동/직원 등)는 더 이상 사용하지 않는다.
...
function normalizeCompanionKey(v){
  ...
  if(['false','0','no','n','아니오','없음','해당없음','미신청'].includes(n))return '';
  return raw;
}
...
function excludedCompanionSet(){
  // v0.9.28: 동반신청 그룹은 별도 제외목록을 사용하지 않는다.
  return new Set();
}

function rebuildAutomaticGroups({persist=false}={}){
  // 수동 대표자 그룹 + 사용자가 직접 수정해 고정한 그룹은 항상 최우선 유지.
  // 자동그룹 우선순위는 "동반신청 → 동일 외부기관"으로 처리한다.
  // 따라서 같은 기관 소속이어도 동반그룹 값이 있으면 동반신청끼리 먼저 하나의 단체가 된다.
  ...
  const overriddenCompanions=new Set(
    manualGroups.filter(g=>g.manualOverride&&g.sourceType==='companion').map(g=>normalizeCompanionKey(g.sourceKey||g.companionGroup)).filter(Boolean)
  );
  const autoGroups=[];
  const autoUsed=new Set();

  // 1) 동반신청 자동그룹. 대표 QR이 아니어도 구성원 누구의 QR을 찍든 같은 그룹으로 인식한다.
  const byCompanion=new Map();
  state.participants.filter(participantActive).forEach(p=>{
    if(manualUsed.has(p.id))return;
    const key=normalizeCompanionKey(p.companionGroup);
    if(!key||overriddenCompanions.has(key))return;
  ...
  // 2) 동반/수동그룹에 속하지 않은 같은 외부기관 참가자를 자동그룹으로 묶는다.
  const byOrg=new Map();
  state.participants.filter(participantActive).forEach(p=>{
...
app.get('/api/health',(req,res)=>{
  ...
  res.json({ok:true,version:'0.9.37',serverTime:nowIso(),uptimeSeconds:Math.round(process.uptime()),participants:state.participants.length,
...
app.get('/api/public/seat-layout',(req,res)=>{
  // 공개 좌석안내는 항상 최종 400석 틀을 사용한다. 서버 상태가 아직 이전 배치여도 21~26번이 비어 보이지 않도록 한다.
  const source=isCurrentHallLayout()?buildHall400Seats():buildEvent400Seats();
  const rows=source.filter(x=>x.enabled!==false).map(x=>({code:x.code,label:displaySeatCode(x.code),row:x.row,side:x.side,number:x.number,displayNumber:x.displayNumber||x.number,section:x.section||'',block:x.block||'',zone:x.zone||''}));
  res.setHeader('Cache-Control','no-store');
  res.json({ok:true,rows});
});


app.get('/raffle-stage',(req,res)=>{
  res.setHeader('Cache-Control','no-store');
  res.sendFile(path.join(ROOT,'public','raffle-stage.html'));
});

app.get('/api/public/raffle-stage',(req,res)=>{
  const fixedPublic=str(req.query.fixed)==='1';
  if(!fixedPublic && str(req.query.k)!==raffleStageKey())return res.status(403).end();
...
app.get('/api/raffle/stage-link',auth,(req,res)=>{
  res.json({ok:true,url:`${req.protocol}://${req.get('host')}/raffle-stage`});
});
...
app.get('/api/bootstrap',auth,(req,res)=>{
  ...
  res.json({ok:true,serverTime:nowIso(),version:'0.9.37',frontendVersion:FRONTEND_VERSION,demoMode:SYSTEM_DEMO_MODE,
...
app.post('/api/participants/onsite',auth,(req,res)=>{
  ...
  let smsQueued=false;
  if(p.phone&&state.settings.checkinSmsEnabled!==false){
    queueAndSendSms(p.phone,checkinMessage(p,'현장 추가 접수로 자유석으로 안내드립니다.'),'onsite',p.id);
    smsQueued=true;
  }
  saveState();res.json({ok:true,participant:p,smsQueued,seatLabel:'자유석'});
});
...
app.post('/api/participants/:id/admin-update',auth,(req,res)=>{
  ...
  if('seatCategory'in b){p.seatCategory=str(b.seatCategory)||'auto';applyStandingPreference(p);}
...
app.get('/api/participants/unassigned',auth,(req,res)=>{
  const rows=state.participants.filter(p=>participantActive(p)&&!p.seat&&p.seatCategory!=='standing')
...
app.post('/api/checkin/test-individual',auth,(req,res)=>{
  const p=findParticipant(req.body?.code);if(!p)return res.status(404).json({ok:false,error:'QR 참가자를 찾을 수 없습니다.'});
  const g=groupForParticipant(p);if(g)return res.status(409).json({ok:false,error:'단체 참가자입니다.',groupRequired:true,participant:p});
  // DRY RUN: 실제 상태/좌석/기념품/SMS는 절대 변경하지 않는다.
  res.json({ok:true,testMode:true,already:Boolean(p.arrived),participant:{...p},wouldSendSms:Boolean(p.phone&&state.settings.checkinSmsEnabled!==false),wouldAssignSeat:false});
});
app.post('/api/checkin/test-group',auth,(req,res)=>{
  rebuildAutomaticGroups({persist:false});
...
app.post('/api/checkin/group',auth,(req,res)=>{
  ...
  const seatPlan={mode:'fixed-seats-only',seats:selected.map(p=>p.seat).filter(Boolean)};
  ...
  if(smsTarget?.phone&&state.settings.checkinSmsEnabled!==false){
    const smsSeatMembers=group.type==='companion'?members:selected;
    const seated=smsSeatMembers.filter(p=>p.seat);
    const seatEntries=seated.map(p=>`${p.name} ${displaySeatCode(p.seat)}`);
    const freeCount=Math.max(0,smsSeatMembers.length-seated.length)+extras;
    const seatLine=seatEntries.length?`배정좌석: ${seatEntries.join(', ')}
`:'';
    const freeLine=freeCount?`자유석: ${freeCount}명
`:'';
    const countLine=group.type==='companion'?`동반그룹 ${members.length}명`:`이번 접수 ${actual}명`;
    const guideLine=seatEntries.length?`좌석배치도(이름별 좌석 함께 표시): ${seatGuideUrl(smsTarget)}
`:'';
    sms=queueAndSendSms(smsTarget.phone,`[남양주시장애인복지관]
${displayName} 현장 접수가 완료되었습니다.
${countLine}
${seatLine}${freeLine}${guideLine}기념품: ${group.type==='companion'?checkCount:actual}명 지급완료
...
app.post('/api/groups/exclusions',auth,(req,res)=>{
  // v0.9.28부터 제외 기준은 행사 운영 정책으로 고정한다.
...
app.post('/api/groups/:id/delete',auth,(req,res)=>{
  ...
  if((g.type==='organization'&&g.auto) || (g.manualOverride&&g.sourceType==='organization')){
    ...
    return res.status(400).json({ok:false,error:'동반신청 그룹은 제외목록을 사용하지 않습니다. 구성원은 동반그룹 수정에서 조정해 주세요.'});
  }else{
    state.groups.splice(i,1);
  }
  rebuildAutomaticGroups({persist:false});
...
app.get('/api/groups/excluded',auth,(req,res)=>{
  res.json({ok:true,organizations:state.settings.excludedOrganizations||[],companions:[]});
});
...
app.put('/api/groups/:id/manage',auth,(req,res)=>{
  ...
  if(wasAuto){
    ...
    g.auto=false;
    g.sourceType=g.type;
    g.sourceKey=g.type==='organization'?normalizeOrg(g.organization):normalizeCompanionKey(g.companionGroup);
...
function applyCanonical400Layout({preserveAssignments=true}={}){
  ...
  state.seats=canonical;state.settings.autoSeatAssignOnCheckin=true;state.meta.seatLayout='EVENT400-V8';state.meta.seatLayoutAppliedAt=nowIso();
...
app.post('/api/seats/apply-event-400',auth,(req,res)=>{
  ...
  adminAudit('400석최신배치적용',{id:'EVENT400-V8',name:'400석 A~L 기존좌석 유지 + M~T 확장 + 휠체어 끝좌석'},null,result);
...
app.get('/api/seats/memories',auth,(req,res)=>{
  res.json({ok:true,rows:(state.seatLayoutMemories||[]).map(x=>({id:x.id,label:x.label,createdAt:x.createdAt,layoutVersion:x.layoutVersion,seats:(x.seats||[]).length}))});
});
app.post('/api/seats/memories',auth,(req,res)=>{
  const m=createSeatLayoutMemory(req.body?.label||'수동 좌석기억');saveState();
  adminAudit('좌석배치기억저장',{id:m.id,name:m.label},null,{createdAt:m.createdAt});
  res.json({ok:true,memory:{id:m.id,label:m.label,createdAt:m.createdAt,layoutVersion:m.layoutVersion}});
});
app.post('/api/seats/memories/:id/restore',auth,(req,res)=>{
  const m=(state.seatLayoutMemories||[]).find(x=>x.id===req.params.id);if(!m)return res.status(404).json({ok:false,error:'좌석기억을 찾을 수 없습니다.'});
  const safety=createSeatLayoutMemory('복원 직전 자동기억');
  const result=restoreSeatLayoutMemory(m);saveState();backupNow('after-seat-memory-restore');
  adminAudit('좌석배치기억복원',{id:m.id,name:m.label},null,result);
  res.json({ok:true,...result,safetyMemoryId:safety.id});
});
app.post('/api/seats/apply-current-hall',auth,(req,res)=>{
  const memory=createSeatLayoutMemory('현장 좌18+우8 312석 적용 전 자동기억');
  ...
  adminAudit('현장좌석배치적용',{id:'ASYM312-V1',name:'A~F 16석 유지 · 뒤 G~O 좌1~18/우19~26 · O열 왼쪽 막힘 · 기타 스탠딩'},null,result);
...
app.get('/api/seats',auth,(req,res)=>{
  ...
  // v0.9.28: 관리자 좌석판은 저장 상태가 구버전이어도 항상 최종 400석 틀을 보여준다.
  // 따라서 M~T 21~26번도 400석 적용 버튼을 누르기 전부터 직접 지정할 수 있다.
  const currentByCode=new Map((state.seats||[]).map(x=>[str(x.code).toUpperCase(),x]));
  const canonical=isCurrentHallLayout()?buildHall400Seats():buildEvent400Seats();
  const rows=canonical.map(base=>{
    const current=currentByCode.get(str(base.code).toUpperCase());
    const s=current?{...base,...current,code:base.code,row:base.row,side:base.side,number:base.number,displayNumber:base.displayNumber,section:base.section,block:base.block,zone:base.zone,autoAssignable:base.autoAssignable,wheelchairAssignable:base.wheelchairAssignable,wheelchairOnly:base.wheelchairOnly,sortOrder:base.sortOrder}:{...base};
  ...
  res.json({
    ...
    layoutNeedsRepair:isCurrentHallLayout() ? (rows.filter(x=>x.enabled!==false).length!==312 || state.meta.seatLayout!=='ASYM312-V2') : (state.meta.seatLayout!=='EVENT400-V8' || rows.length!==400 ||
...
app.post('/api/seats/:code/add-participant',auth,(req,res)=>{
  ...
  if(seat.staffOnly)return res.status(400).json({ok:false,error:'스태프 자유석은 참가자에게 지정할 수 없습니다.'});
...
app.post('/api/seats/:code/assign',auth,(req,res)=>{
  ...
  if(p.seatCategory==='standing')return res.status(400).json({ok:false,error:'스탠딩 참가자는 좌석을 지정할 수 없습니다. 좌석 분류를 변경해 주세요.'});
...
app.post('/api/seats/final-auto-assign',auth,(req,res)=>{
  let vip=0,guest=0,wheelchair=0,guardians=0,companions=0,standing=0;
  const active=state.participants.filter(p=>participantActive(p)&&!p.onsite);
  active.filter(p=>p.seatCategory==='standing').forEach(p=>{applyStandingPreference(p);standing++;});
  ...
  const manualLocked=active.filter(p=>p.seat&&p.seatLocked&&!['vip','guest','standing'].includes(p.seatCategory)&&!p.wheelchairUser&&p.seatCategory!=='wheelchair').length;
  const unassignedGeneral=active.filter(p=>!p.seat&&p.seatCategory!=='standing'&&p.seatCategory!=='vip'&&p.seatCategory!=='guest'&&!p.wheelchairUser&&p.seatCategory!=='wheelchair').length;
  const result={vip,guest,wheelchair,guardians,companions,standing,manualLocked,unassignedGeneral};
...
function eligibleRafflePool(filter='all'){
  ...
  if(filter==='freeSeat')pool=pool.filter(p=>!p.seat);
...
app.get('/api/raffle/pool',auth,(req,res)=>{
  const filter=str(req.query?.filter||'all');
  const pool=eligibleRafflePool(filter);
  res.json({ok:true,filter,count:pool.length,arrivedTotal:state.participants.filter(p=>p.arrived&&participantActive(p)).length,freeSeat:pool.filter(p=>!p.seat).length,onsite:pool.filter(p=>p.onsite).length});
});
...
app.post('/api/raffle/remote/start',auth,(req,res)=>{
  ...
  const productNo=str(req.body?.productNo),count=Math.max(1,Math.min(5,num(req.body?.count,1))),filter=str(req.body?.filter||'all');
...
app.post('/api/raffle/draw',auth,(req,res)=>{
  const filter=str(req.body?.filter||'all'),productNo=str(req.body?.productNo),count=Math.max(1,Math.min(5,num(req.body?.count,1)));
  const product=state.rouletteProducts.find(x=>str(x.number)===productNo)||{number:productNo||'custom',name:str(req.body?.productName)||'행운상품',quantity:999,enabled:true};
  const remaining=productRemaining(product);
  if(remaining<count)return res.status(400).json({ok:false,error:`${product.name} 남은 수량이 ${remaining}개입니다.`});
  const pool=eligibleRafflePool(filter);
...
function previewParticipantCsv(buffer,name='participants.csv'){
  ...
  for(let i=0;i<rows.length;i++){
    ...
    planned.push({
      ...
      seatCategory:(()=>{const v=str(r['좌석분류']||r['좌석구분']||r['좌석유형']);return v==='스탠딩'?'standing':(v==='VIP'?'vip':(v==='내빈'?'guest':(v==='휠체어 우선'?'wheelchair':(found?.seatCategory||'auto'))));})(),
...
app.post('/api/import/participants-csv/confirm',auth,(req,res)=>{
  ...
  for(const x of h.csvMerge.planned){
    ...
    if(existing){
      ...
      if(x.seatCategory){existing.seatCategory=x.seatCategory;applyStandingPreference(existing);}
      ...
      const p={id:x.id,receptionNo:x.receptionNo,name:x.name,phone:x.phone,organization:x.organization,seat:x.seat,
        ...
        active:true,requestedCount:1,wheelchairUser:false,wheelchairCount:0,usesCenter:false,disabledPerson:false,companionGroup:x.companionGroup||'',participationStatus:'참여',seatCategory:x.seatCategory||'auto',seatLocked:Boolean(x.seat),giftReceived:x.giftReceived,onsite:x.onsite};
...
app.get('/api/export/participants.csv',auth,(req,res)=>{
  ...
  const headers=['단체구분','단체명','대표여부','동반그룹','접수번호','QR고유코드','이름','연락처','소속기관','좌석','좌석분류','도착여부','도착시각','기념품','현장접수'];
  const rows=rowsMeta.map(({p,g})=>[
    ...
    p.receptionNo,p.id,p.name,p.phone,p.organization,p.seat,({standing:'스탠딩',vip:'VIP',guest:'내빈',wheelchair:'휠체어 우선',auto:'자동/일반'}[p.seatCategory||'auto']||p.seatCategory||'자동/일반'),p.arrived?'Y':'N',p.arrivedAt||'',p.giftReceived?'Y':'N',p.onsite?'Y':'N'
  ]);
...
// v0.9.34: 좌석 동결 모드. 배포 시 좌석 구조/참가자 좌석을 자동변환하지 않는다.
(function freezeSeatAssignmentMode(){
  try{
    if(state.settings.autoSeatAssignOnCheckin!==false){
      state.settings.autoSeatAssignOnCheckin=false;
      saveState();
    }
  }catch(e){console.error('[SEAT FREEZE v0.9.34]',e);}
})();

app.listen(PORT,'0.0.0.0',()=>console.log(`NYJWEL Admin v0.9.37 · :${PORT}`));
