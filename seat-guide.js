...
function parseSeat(raw){
  ...
  let m=raw.match(/^([A-W])(\d{1,2})$/);if(m)return{row:m[1],n:Number(m[2]),side:''};
...
function render(mySeats,layout){
  ...
  const hall=rows.length===402||rows.some(s=>s.row==='W')||rows.some(s=>s.staffOnly)||rows.some(s=>s.row==='STAFF');
  if(hall){
    const frontRow=(row)=>{const m=new Map((byRow.get(row)||[]).map(s=>[Number(s.displayNumber||s.number),s]));return `<div class="front-row"><b class="row-label">${row}</b><div class="eight">${Array.from({length:8},(_,i)=>dotSeat(m.get(i+1),mines,i+1)).join('')}</div><span class="runway"></span><div class="eight">${Array.from({length:8},(_,i)=>dotSeat(m.get(i+9),mines,i+9)).join('')}</div></div>`};
    const blockRow=(row)=>{const m=new Map((byRow.get(row)||[]).map(s=>[Number(s.displayNumber||s.number),s]));return `<div class="rear-row"><b class="row-label">${row}</b><div class="eighteen">${Array.from({length:18},(_,i)=>dotSeat(m.get(i+1),mines,i+1)).join('')}</div></div>`};
    const af='ABCDEF'.split('').map(frontRow).join('');
    const left='GHIJKLMN'.split('').map(blockRow).join('');
    const right='OPQRSTUVW'.split('').map(blockRow).join('');
    $('#seatMap').innerHTML=af+`<div class="rear-start">앞 6줄 A~F · 내빈석 유지</div><div class="dual-mini"><div>${left}<div class="rear-start">왼쪽 추가석 G~N · 각 18석</div></div><div>${right}<div class="rear-start">오른쪽 추가석 O~W · 각 18석</div></div></div><div class="rear-start">기타 구역은 스탠딩</div>`;
    return;
  }
