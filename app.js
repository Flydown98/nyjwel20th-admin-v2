...
async function refreshDashboard(){
  ...
  $('#statusBadge').textContent='연결됨 · v0.9.31';$('#statusBadge').classList.add('ok');$('#roleBadge').textContent=d.roleLabel||currentRole;
...
async function loadSeats(){
  try{
    ...
    let html='';
    if(String(d.layoutVersion||'').startsWith('ASYM402-')){
      ...
      const blockRow=(row)=>{const rs=(byRow.get(row)||[]),map=new Map(rs.map(s=>[+s.displayNumber||+s.number,s]));return `<div class="rear-seat-row"><div class="seat-row-label">${row}</div><div class="seat-block eighteen">${Array.from({length:18},(_,i)=>seatCellHtml(map.get(i+1),i+1)).join('')}</div></div>`};
      const front='ABCDEF'.split('').map(frontRow).join('');
      const left='GHIJKLMN'.split('').map(blockRow).join('');
      const right='OPQRSTUVW'.split('').map(blockRow).join('');
      html=`${d.layoutNeedsRepair?'<div class="seat-layout-warning"><strong>현장 좌석틀 저장 확인 필요</strong><span>비대칭 402석 구조를 서버 상태에 저장할 수 있습니다.</span></div>':''}<div class="seat-stage-label">무대 / 스테이지</div><div class="front-layout-label"><span>좌측</span><span>런웨이</span><span>우측</span></div>${front}<div class="runway-end-cap">앞 6줄 내빈석 · A~F 1~16 유지</div><div class="dual-rear-wrap"><div><div class="runway-end-cap side-note">왼쪽 추가석 · G~N 각 18석 · 총 144석</div>${left}</div><div><div class="runway-end-cap side-note">오른쪽 추가석 · O~W 각 18석 · 총 162석</div>${right}</div></div><div class="runway-end-cap">총 좌석 402석 · 그 외 구역은 스탠딩 운영</div>`;
    }else{
...
if('serviceWorker' in navigator){
  window.addEventListener('load',()=>navigator.serviceWorker.register('/sw.js?v=0.9.31',{updateViaCache:'none'}).then(r=>r.update()).catch(()=>{}));
}
